...
class NASNetwork(nn.Module):
    def __init__(self, input_size, num_cells, activation):
        super(NASNetwork, self).__init__()
        self.cells = nn.ModuleList()
        current_features = input_size
        for _ in range(num_cells):
            cell = NASCell(current_features, current_features * 2, activation)
...
# 实现 DARTS 算法
class DARTS:
    def __init__(self, model, criterion, optimizer, scheduler, warmup_scheduler, device, history, accumulation_steps=1):
        self.model = model
...
def objective(trial, train_loader, val_loader, device, tuning_dir, column):
    ...
    # 构建 NASNetwork
    model = NASNetwork(input_size=X_train_tensor.shape[1], num_cells=num_cells, activation=activation).to(device)
    ...
    # 设置 DARTS
    darts = DARTS(model, criterion, optimizer, scheduler, warmup_scheduler, device, history, accumulation_steps=accumulation_steps)
    ...
    # 保存最佳模型
    if val_loss < trial.user_attrs.get('best_val_loss', float('inf')):
        trial.set_user_attr('best_val_loss', val_loss)
        os.makedirs(os.path.join(trial_dir, 'best_model'), exist_ok=True)
        torch.save(model.state_dict(), os.path.join(trial_dir, 'best_model', 'best_model.pth'))
        ...
        if os.path.exists(train_losses_file) and os.path.exists(val_losses_file):
            ...
            if os.path.exists(params_file):
                with open(params_file, 'r') as f:
                    params = json.load(f)
                key_params = {k: v for k, v in params.items() if k in ['num_cells', 'learning_rate', 'activation', 'activation_name']}
                if 'activation_name' in key_params:
                    key_params['activation'] = key_params.pop('activation_name')
...
for column in target_columns:
    ...
    best_model = NASNetwork(input_size=X_train_tensor.shape[1],
                            num_cells=best_trial.params['num_cells'],
