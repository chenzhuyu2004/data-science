...
# 可视化模型结构
def visualize_model_structure(model, column, save_path):
    layers_info = []
    total_params = 0
    for i, layer in enumerate(model.cells):
        in_features = layer.fc.in_features
        out_features = layer.fc.out_features
        params = sum(p.numel() for p in layer.parameters() if p.requires_grad)
        layers_info.append({
            'Layer': f'Cell {i+1}',
            'Input Dim': in_features,
            'Output Dim': out_features,
            'Params': params
        })
        total_params += params
    # 添加最后一层的线性层
    in_features = model.cells[-1].fc.out_features
    out_features = 1
    params = sum(p.numel() for p in model.fc.parameters() if p.requires_grad)
    layers_info.append({
        'Layer': 'Output Layer',
        'Input Dim': in_features,
        'Output Dim': out_features,
    ...
    fig = go.Figure(go.Bar(
        x=[info['Layer'] for info in layers_info],
        y=[info['Output Dim'] for info in layers_info],
        text=[f"Params: {info['Params']:,}<br>Input Dim: {info['Input Dim']}<br>Output Dim: {info['Output Dim']}" for info in layers_info],
        hovertemplate='%{text}',
        marker_color='lightblue'
    ))

    fig.update_layout(
        title=f"{column} - Model Structure",
        xaxis_title='Layer',
...
for column in target_columns:
    ...
    # 为每个目标变量创建子目录
    column_dir = os.path.join(main_training_dir, column)
    os.makedirs(column_dir, exist_ok=True)

    # 设置目录
    tuning_dir = os.path.join(column_dir, 'tuning_results')
    ...
    plt.savefig(os.path.join(column_dir, f'{column}_best_model_loss_lr.png'))
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(column_dir, f'{column}_residuals_distribution.png'))
    plt.close()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(column_dir, f'{column}_best_model_R2.png'))
    plt.close()

    # 绘制所有trial的损失曲线图
    trial_dirs = [os.path.join(tuning_dir, f'trial_{trial.number}') for trial in study.trials]
    plot_trials_loss_data(trial_dirs, column_dir, column)

    # 可视化模型结构并保存
    visualize_model_structure(best_model, column, os.path.join(column_dir, f'{column}_model_structure.html'))
